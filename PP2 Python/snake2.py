...
import random
import time
#sys ради функции exit - чтобы завершать скрипт при ошибке или game over'e
#time - ради функции sleep, чтобы экран игры немного задержался при game over'e.
class Game():
    def __init__(self):
    ...
    def init_and_check_for_errors(self):
        ...
        """На строке 36 в переменной check_errors будет кортеж вида 
        (6, 0) - (количество завершенных тасков, количество ошибок), 
        поэтому на строке 37 берется второй элемент кортежа и проверяется на количество ошибок. 
        В функции event_loop и в фукции game_over я сначала убиваю окно pygame функцией quit(), 
        а затем уже выхожу из скрипта sys.exit'ом."""
    ...
    def show_score(self, choice=1):
        """переменная choice нужна чтобы определить как выводить результат,
        слева сверху в течение игры 84 строчка или по центру в момент завершения 88 строчка"""
...
class Snake():
    ...
    def snake_body_mechanism(
        ...
        """ В функции snake_body_mechanism, в которой описан механизм изменения тела змеи, 
        есть важный момент - в строке 153 обязательно создавать новый список (делать list(self.snake_head_pos)), 
        иначе (если сделать просто self.snake_body.insert(0, self.snake_head_pos)), 
        будет передан не новый список, а тот же самый список snake_head_pos,
        и через несколько итераций мы будем управлять одной точкой, после чего игра завершится game_over'ом."""
        ...
        if (self.snake_head_pos[0] == food_pos[0] and
            ...
            """Позиция еды на строке 163 задается таким образом, чтобы x и y еды были всегда кратны 10,
            чтобы всегда была возможность ее захватить. Начало координат находится в левом верхнем углу,
            причем ось x напрвлено вправо, а ось y - вниз, поэтому на строках 134,136,138 и 140 
            координаты изменяются соответствующим образом."""
...
"""Тут все понятно, единственное, в методе draw_food используется метод rect(),
который принимает 3 аргумента: поверхность, цвет и объект, в данном случае Rect(x,y, sizex, sizey)."""
...
в котором последовательно вызыаем описанные методы классов с соответствующими параметрами:"""
while True:
